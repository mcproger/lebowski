...
def to_milliseconds(timestamp: float) -> int:
    return int(timestamp) * 1000


def operations_piechar_url(
    session: str, from_datetime: datetime = None, to_datetime: datetime = None
) -> str:
    if from_datetime:
    ...
    return OPERATIONS_PIECHART_URL.format(
        signed_up_session_id=session,
        from_timestamp=to_milliseconds(from_timestamp),
...
def convert_raw_tinkoff_data(raw_data: dict) -> dict:
    ...
    return {element['spendingCategory']['name']: element['amount']['value'] for element in raw_data}
